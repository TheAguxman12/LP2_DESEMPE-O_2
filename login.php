...
if ($_POST){
  ...
  $yourUsername = isset($_POST['yourUsername']) ? $_POST['yourUsername'] : '';
  $yourPassword = isset($_POST['yourPassword']) ? $_POST['yourPassword'] : '';

  $simple = $connection->prepare("SELECT * FROM USUARIOS WHERE usuario = :yourUsername");
  $simple->bindParam(":yourUsername", $yourUsername);
  $simple->execute();

  $resultado = $simple-> fetch(PDO::FETCH_ASSOC);
  
  if($resultado && password_verify($yourPassword, $resultado['clave'])){
    $_SESSION['$yourUsername'] = $resultado['$yourUsername'];
    $_SESSION['login'] = true;
   header("Location: index.php");
  }else{
    $alert_messenge = '<div class="alert alert-danger" role="alert">El usuario y/o contraseña son incorrectos</div>';
  }
